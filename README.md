...
- [Available Builders (ZynqMP)](#available-builders-(zynqmp))
  ...
  - [ZynqMP_AlpineLinux_RootFS_Builder](#zynqmp_alpinelinux_rootfs_builder)
    - [Block Configuration](#block-configuration-1)
    - [External Source Files](#external-source-files-1)
  - [ZynqMP_AMD_ATF_Builder](#zynqmp_amd_atf_builder)
    - [Block Configuration](#block-configuration-2)
    - [External Source Files](#external-source-files-2)
  - [ZynqMP_AMD_Devicetree_Builder](#zynqmp_amd_devicetree_builder)
    - [Block Configuration](#block-configuration-3)
    - [External Source Files](#external-source-files-3)
  - [ZynqMP_AMD_FSBL_Builder](#zynqmp_amd_fsbl_builder)
    - [Block Configuration](#block-configuration-4)
    - [External Source Files](#external-source-files-4)
  - [ZynqMP_AMD_Image_Builder](#zynqmp_amd_image_builder)
    - [Block Configuration](#block-configuration-5)
    - [External Source Files](#external-source-files-5)
  - [ZynqMP_AMD_Kernel_Builder](#zynqmp_amd_kernel_builder)
    - [Block Configuration](#block-configuration-6)
    - [External Source Files](#external-source-files-6)
  ...
  - [ZynqMP_AMD_PetaLinux_RootFS_Builder](#zynqmp_amd_petalinux_rootfs_builder)
    - [Block Configuration](#block-configuration-7)
    - [External Source Files](#external-source-files-7)
  - [ZynqMP_AMD_PMUFW_Builder](#zynqmp_amd_pmufw_builder)
    - [Block Configuration](#block-configuration-8)
    - [External Source Files](#external-source-files-8)
  - [ZynqMP_AMD_UBoot_SSBL_Builder](#zynqmp_amd_uboot_ssbl_builder)
    - [Block Configuration](#block-configuration-9)
    - [External Source Files](#external-source-files-9)
  - [ZynqMP_AMD_Vivado_Hog_Builder](#zynqmp_amd_vivado_hog_builder)
    - [Block Configuration](#block-configuration-10)
    - [External Source Files](#external-source-files-10)
  - [ZynqMP_AMD_Vivado_IPBB_Builder](#zynqmp_amd_vivado_ipbb_builder)
    - [Block Configuration](#block-configuration-11)
    - [External Source Files](#external-source-files-11)
  - [ZynqMP_AMD_Vivado_logicc_Builder](#zynqmp_amd_vivado_logicc_builder)
    - [Block Configuration](#block-configuration-12)
    - [External Source Files](#external-source-files-12)
  - [ZynqMP_BusyBox_RAMFS_Builder](#zynqmp_busybox_ramfs_builder)
    - [Block Configuration](#block-configuration-13)
    - [External Source Files](#external-source-files-13)
...
SoCks does not require any external files in order to build this block. However, it is possible to create patches for the source repo. Depending on the Vivado project, the FSBL project may use the _libmetal_ library. This library uses a rather complex build infrastructure located in `zynqmp_fsbl_bsp/psu_cortexa53_0/libsrc/libmetal_*/build_libmetal/`. It is recommended to add a line similar to the following to the gitignore file in the root directory of the FSBL project repository to excluse this infrastructure from Git tracking:
```
zynqmp_fsbl_bsp/psu_cortexa53_0/libsrc/libmetal_v2_4/build_libmetal/
```

The following template packages are available:
...
SoCks does not require any external files in order to build this block. However, it is possible to create patches for the source repo. Depending on the Vivado project, the PMU firmware project may use the _libmetal_ library. This library uses a rather complex build infrastructure located in `zynqmp_pmufw_bsp/psu_pmu_0/libsrc/libmetal_*/build_libmetal/`. It is recommended to add a line similar to the following to the gitignore file in the root directory of the FSBL project repository to excluse this infrastructure from Git tracking:
```
zynqmp_pmufw_bsp/psu_pmu_0/libsrc/libmetal_v2_4/build_libmetal/
```
...
- **source**: The source of the block. Options are:
  - **build**: Build the block locally
  - **import**: Import an already built block package
- **builder**: The builder to be used to build this block. For this builder always `ZynqMP_AMD_UBoot_SSBL_Builder`.
- **project -> build_srcs -> source**: The source to be used to build this block in URI format. Options are:
  - The URL of a git repository. In this case the string must start with `https://` or `ssh://`.
  - The path to a local folder, e.g. if the repo was checked out manually. In this case the string must start with `file://`.
- **project -> build_srcs -> branch [optional]**: Specifies the branch of the source repo to be used. Only permitted if *project -> build_srcs -> source* contains the URL of a git repo.
...
rootfs:
  ...
  project:
    ...
    custom_pkgs:
      - name: "kernel-modules-socks"
        install: false
        build_time_files:
          - src_block: "kernel"
            src_name: "kernel_modules.tar.gz"
            dest_path: "/tmp/kernel_module_update"
            dest_owner_group: "root:root"
      - name: "vivado-bitfile"
        install: true
        build_time_files:
          - src_block: "vivado"
            src_name: "xsa_extracted/*.bit"
            dest_path: "/lib/firmware"
            dest_name: "pl_bitfile.bit"
            dest_owner_group: "root:root"
            dest_permissions: "u=rw,go=r"
    build_time_fs_layer:
      - src_block: "vivado"
...
- **project -> addl_ext_pkgs [optional]**: A list of additional deb packages to be installed from external \*.deb files into the root file system. List entries must be strings in URI format. Supported are:
  - The URL to a packages online. The string must start with `https://` or `http://`.
  - The file URI of a local packages. The string must start with `file://`. Local packages should not be included in the Git repo of the SoCks project. It is recommended to use absolute paths.
- **project -> custom_pkgs [optional]**: A list of dicts describing custom packages to be built by SoCks. The intended use case is to build packages that contain files and folders generated by other blocks. If this is not the case, it is recommended to build packages independently of SoCks and install them via `addl_ext_pkgs`. Each package listed here must have a corresponding folder in `src/rootfs/resources/package_src_trees` named after the package. The contents of this folder must be set up so that `dpkg-deb --build` can operate on it. This means that it must contain at least a folder named `DEBIAN`, which contains a `control` file with a suitable Debian package configuration.
- **project -> custom_pkgs [optional] -> [N] -> name**: The name of the package
- **project -> custom_pkgs [optional] -> [N] -> install**: A binary parameter that specifies whether the package should be installed in the file system during the build process
- **project -> custom_pkgs [optional] -> [N] -> build_time_files**: A list of dicts describing files and folders generated by other blocks that are to be included in the package. The elements in this list correspond to those in `build_time_fs_layer`. Accordingly, you can find more information about the structure of the elements in the section on `build_time_fs_layer`.
- **project -> build_time_fs_layer [optional]**: A list of dicts describing files and folders generated by other blocks and are to be added to the root file system.
- **project -> build_time_fs_layer -> [N] -> src_block**: The ID of the block that generates this file or folder.
- **project -> build_time_fs_layer -> [N] -> src_name**: The name of the file or folder in the block package of the source block.
- **project -> build_time_fs_layer -> [N] -> dest_path**: The target path in the root file system where this file or folder is to be placed.
...
- **project -> import_src [optional]**: The pre-built block package to be imported for this block. This information is only used if the value of *source* is *import*. Options are:
  ...
  - The file URI of a local file. In this case the string must start with `file://`.
- **project -> add_build_info**: A binary parameter that specifies whether build-related information should be built into the root file system. If it is set to `true`, SoCks creates the file `/etc/fs_build_info` with build related information in the root file system.
- **project -> static_kernel_modules**: A binary parameter that specifies whether kernel modules are installed in the file system statically. If they are not installed statically, they should be installed with a custom package.
- **project -> dependencies**: A dict with all dependencies required by this builder to build this block. The keys of the dict are block IDs. The values of the dict are paths to the respective block packages. All paths are relative to the SoCks project directory. In almost all cases, the values from the example configuration can be used.
...
- **source**: The source of the block. Options are:
  - **build**: Build the block locally
  - **import**: Import an already built block package
- **builder**: The builder to be used to build this block. For this builder always `ZynqMP_Ubuntu_RootFS_Builder`.
- **project -> release**: The release version of Ubuntu to be built. Options are:
  - bookworm
...
SoCks requires external files in order to build this block. The following template packages are available:
- **universal**: Contains universal template files to build a Ubuntu root file system. The optional file `mod_base_install.sh` allows to modify the base root file system after all packages have been added, but before any other modifications have been made to it. The optional file `conclude_install.sh` allows to finalize the creation of the root file system. The optional folder `predefined_fs_layers` allows to add static layers that are added to the base root file system. Every layer requires a shell script that is used to add the layer.
