...
set -x
dnf $dnf_default_parameters remove iw*firmware* --setopt=tsflags=noscripts
dnf $dnf_default_parameters autoremove

# Install GPG key for CERN repos in the container
#curl -o "/etc/pki/rpm-gpg/RPM-GPG-KEY-kojiv2" "https://gitlab.cern.ch/api/v4/projects/141918/repository/files/src%2FRPM-GPG-KEY-kojiv2/raw?ref=main"
...
if [ ! -e "$key_path/RPM-GPG-KEY-kojiv2" ]; then
    echo "CERN GPG key not found. The key can be downloaded from here: https://gitlab.cern.ch/linuxsupport/rpms/cern-gpg-keys/"
    exit 1
fi
...
dnf -y --nodocs --verbose --releasever=$release_ver --forcearch=$target_arch --installroot=$install_root config-manager --set-enabled powertools
...
# Install GPG key for CERN repos in the target rootfs
cp "/etc/pki/rpm-gpg/RPM-GPG-KEY-kojiv2" "$install_root/$key_path/"
