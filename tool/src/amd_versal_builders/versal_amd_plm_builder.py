...
from amd_versal_builders.versal_amd_plm_model import Versal_AMD_PLM_Model
...
class Versal_AMD_PLM_Builder(AMD_Builder):
    ...
    def build_plm(self):
        ...
        with self._build_log.timestamp(identifier=f"function-{inspect.currentframe().f_code.co_name}-success"):
            ...
            (self._output_dir / "plm.elf").symlink_to(
                self._vitis_workspace_dir / "versal_plm" / "build" / "versal_plm.elf"
            )
