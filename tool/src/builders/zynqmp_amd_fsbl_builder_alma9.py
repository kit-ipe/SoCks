...
class ZynqMP_AMD_FSBL_Builder_Alma9(AMD_Builder):
    ...
    def __init__(self, project_cfg: dict, project_cfg_files: list, socks_dir: pathlib.Path, project_dir: pathlib.Path, block_id: str = 'fsbl', block_description: str = 'Build the First Stage Boot Loader (FSBL) for ZynqMP devices'):
        ...
        if self._pc_block_source == 'build':
            self.block_cmds['prepare'].extend([self.build_container_image, self.import_dependencies, self.import_xsa, self.create_fsbl_project, self.apply_patches])
    ...
    def create_fsbl_project(self):
        ...
        if self._pc_container_tool  in ('docker', 'podman'):
            try:
                ...
                pretty_print.print_error(f'An error occurred while creating the FSBL project: {e}')
                sys.exit(1)
        ...
        ZynqMP_AMD_FSBL_Builder_Alma9._run_sh_command(['git', '-C', str(self._source_repo_dir), 'switch', '-c', self._git_local_ref_branch])
        ...
        ZynqMP_AMD_FSBL_Builder_Alma9._run_sh_command(['git', '-C', str(self._source_repo_dir), 'switch', '-c', self._git_local_dev_branch])
    ...
    def build_fsbl(self):
        ...
        if not ZynqMP_AMD_FSBL_Builder_Alma9._check_rebuild_required(src_search_list=self._project_cfg_files + [self._patch_dir, self._source_repo_dir], src_ignore_list=[self._source_repo_dir / 'executable.elf'], out_search_list=[self._source_repo_dir / 'executable.elf']):
        ...
        if self._pc_container_tool  in ('docker', 'podman'):
            try:
                # Run commands in container
                ZynqMP_AMD_FSBL_Builder_Alma9._run_sh_command([self._pc_container_tool , 'run', '--rm', '-it', '-v', f'{self._pc_xilinx_path}:{self._pc_xilinx_path}:ro', '-v', f'{self._xsa_dir}:{self._xsa_dir}:Z', '-v', f'{self._repo_dir}:{self._repo_dir}:Z', '-v', f'{self._output_dir}:{self._output_dir}:Z', self._container_image, 'sh', '-c', fsbl_build_commands])
            except Exception as e:
                pretty_print.print_error(f'An error occurred while building the FSBL: {e}')
                sys.exit(1)
        elif self._pc_container_tool  == 'none':
            # Run commands without using a container
