...
class ZynqMP_AMD_PetaLinux_RootFS_Builder_Alma8(Builder):
    ...
    def __init__(
        ...
        try:
            self.block_configuration = ZynqMP_AMD_PetaLinux_RootFS_Model(**project_cfg)
    ...
    def import_prebuilt(self):
        ...
        if self._pc_project_prebuilt is None:
            pretty_print.print_error(
                f"The property blocks/{self.block_id}/project/pre-built is required to import the block, but it is not set."
            )
            sys.exit(1)
