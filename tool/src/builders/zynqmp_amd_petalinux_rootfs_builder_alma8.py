...
class ZynqMP_AMD_PetaLinux_RootFS_Builder_Alma8(Builder):
    ...
    def __init__(self, project_cfg: dict, project_cfg_files: list, socks_dir: pathlib.Path, project_dir: pathlib.Path, block_id: str = 'rootfs', block_description: str = 'Build an AMD PetaLinux root file system'):
        ...
        if self._pc_block_source == 'build':
            ...
            self.block_cmds['build'].extend([self.build_base_rootfs, self.add_kmodules, self.build_archive, self.export_block_package])
            ...
            self.block_cmds['build'].extend(self.block_cmds['prepare'])
            self.block_cmds['build'].extend([self.import_prebuilt, self.add_kmodules, self.build_archive, self.export_block_package])
