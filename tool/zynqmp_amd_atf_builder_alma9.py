...
class ZynqMP_AMD_ATF_Builder_Alma9(builder.Builder):
    ...
    def build_atf(self):
        ...
        if ZynqMP_AMD_ATF_Builder_Alma9._check_rebuilt_required(src_search_list=[self._patch_dir, self._source_repo_dir], src_ignore_list=[self._source_repo_dir / 'build'], out_search_list=[self._source_repo_dir / 'build/zynqmp/release/bl31/bl31.elf', self._source_repo_dir / 'build/zynqmp/release/bl31.bin']):
            pretty_print.print_build('Building the ATF...')

            if self._container_tool in ('docker', 'podman'):
                try:
                    # Run build commands in container
                    ZynqMP_AMD_ATF_Builder_Alma9._run_sh_command([self._container_tool, 'run', '--rm', '-it', '-v', f'{str(self._repo_dir)}:{str(self._repo_dir)}:Z', '-v', f'{str(self._output_dir)}:{str(self._output_dir)}:Z', self._container_image, 'sh', '-c', atf_build_commands])
