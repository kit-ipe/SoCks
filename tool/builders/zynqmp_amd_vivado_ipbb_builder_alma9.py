...
class ZynqMP_AMD_Vivado_IPBB_Builder_Alma9(AMD_Builder):
    ...
    def __init__(self, project_cfg: dict, project_cfg_files: list, socks_dir: pathlib.Path, project_dir: pathlib.Path):
        block_id = 'vivado'
        block_description = 'Build an AMD/Xilinx Vivado Project with IPbus Builder (IPBB)'
        ...
        if self._pc_project_sources is None:
            pretty_print.print_error(f'Builder {self.__class__.__name__} expects an array in blocks/{self.block_id}/project/build-srcs.')
            sys.exit(1)
