...
class AlpineLinux_RootFS_Builder(File_System_Builder):
    """
        ...
        with self._build_log.timestamp(identifier=f"function-{inspect.currentframe().f_code.co_name}-success"):
            ...
            for user in self.block_cfg.project.users:
                ...
                if user.name != "root":
                    add_user_str = (
                        add_user_str
                        + f"if id {user.name} &>/dev/null; then deluser --remove-home {user.name}; fi && "  # Delete user account if it already exists
                        + f"adduser -D {user.name} && "
                    )
