...
class File_System_Custom_Package_Model(BaseModel):
    model_config = ConfigDict(extra="forbid", strict=True)

    name: Annotated[str, StringConstraints(pattern=r"^[a-z0-9-+.]+$")] = Field(
        default=..., description="Name of the package"
    )
    install: bool = Field(default=..., description="Whether the package is to be installed in the file system")
    build_time_files: Optional[list[File_System_Installable_Build_Time_Item_Model]] = Field(
        default=[],
        description="List of files and directories to be added that were generated by other blocks at build time",
    )
