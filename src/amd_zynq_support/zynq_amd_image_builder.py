...
from amd_zynq_support.zynq_amd_image_model import Zynq_AMD_Image_Model


class Zynq_AMD_Image_Builder(AMD_Builder):
    """
    ...
    def __init__(
        ...
        block_id: str = "image",
        block_description: str = "Build the boot image for Zynq devices",
        model_class: type[object] = Zynq_AMD_Image_Model,
    ):
        ...
        elif "zImage" == self.block_cfg.project.uboot_image_kernel:
            self._uboot_image_kernel = self._dependencies_dir / "kernel/zImage"
        else:
        ...
        elif "zImage" == self.block_cfg.project.boot_image_kernel:
            self._boot_image_kernel = self._dependencies_dir / "kernel/zImage"
    ...
    def linux_img_legacy(self):
        """
        Creates a Linux image that can be loaded from U-Boot in the legacy boot mode.
        ...
        with self._build_log.timestamp(identifier=f"function-{inspect.currentframe().f_code.co_name}-success"):
            ...
            # Remove old build artifacts
            (self._output_dir / "uImage").unlink(missing_ok=True)

            pretty_print.print_build("Building legacy Linux image...")

            linux_img_build_commands = [
                f'mkimage -A arm -O linux -T kernel -C none -a 0x200000 -e 0x200000 -n "SoCks Linux Kernel" -d {self._dependencies_dir}/kernel/zImage {self._output_dir}/uImage',
            ]
    ...
    def boot_img(self):
        ...
        with self._build_log.timestamp(identifier=f"function-{inspect.currentframe().f_code.co_name}-success"):
            ...
            boot_img_build_commands = [
                ...
                f"bootgen -arch zynq -image {self._work_dir}/bootgen.bif -o {self._output_dir}/BOOT.BIN",
            ]
