...
class RaspberryPi_Kernel_Builder(ZynqMP_AMD_Kernel_Builder):
    ...
    def init_repo(self):
        ...
        if self.project_cfg.project.rpi_model == "RPi_4B":
            create_defconfig_commands.append("make bcm2711_defconfig")
        elif self.project_cfg.project.rpi_model == "RPi_5":
            create_defconfig_commands.append("make bcm2712_defconfig")
        else:
            raise ValueError(
    ...
    def create_config_snippet(self):
        ...
        if self.project_cfg.project.rpi_model == "RPi_4B":
            self._create_config_snippet(
                cross_comp_prefix="aarch64-linux-gnu-", arch="arm64", defconfig_target="bcm2711_defconfig"
            )
        elif self.project_cfg.project.rpi_model == "RPi_5":
            self._create_config_snippet(
                cross_comp_prefix="aarch64-linux-gnu-", arch="arm64", defconfig_target="bcm2712_defconfig"
            )
        else:
            raise ValueError(
                f"The following Raspberry Pi Model is not supported: '{self.project_cfg.project.rpi_model}'"
